...
export function grokRuntimeSignature(rt) {
  return [
    ...
    // Include backend so Claude→messages / GPT→chat_completions flips restart ACP.
    rt.isThirdParty ? `backend:${inferThirdPartyApiBackend(rt.model)}` : 'backend:',
...
/**
 * Pick Grok `api_backend` for a third-party model id.
 *
 * Claude models must use Anthropic Messages (`messages`). Gateways like dmxapi
 * (new_api) validate Claude traffic as Anthropic and reject OpenAI-style
 * `image_url` blocks inside tool_result when the client speaks chat_completions
 * but the tool layer returns vision content (e.g. reading a pasted jpg).
 *
 * @param {string} model
 * @returns {'messages' | 'chat_completions'}
 */
export function inferThirdPartyApiBackend(model) {
  const m = String(model || '').trim().toLowerCase();
  if (!m) return 'chat_completions';
  // claude-*, anthropic/claude-*, …opus/sonnet/haiku aliases that include claude
  if (m.includes('claude') || m.startsWith('anthropic/')) return 'messages';
...
export function buildThirdPartyConfigToml(opts) {
  ...
  const apiBackend = inferThirdPartyApiBackend(model);
  ...
  // Still set for messages: harmless, and keeps one path for gateway quirks.
  const extraHeaders =
    apiBackend === 'messages'
      ? [
          // dmxapi accepts Bearer (api_key) or x-api-key; version header is required
          // by Anthropic Messages protocol on most gateways.
          `extra_headers = { "anthropic-version" = "2023-06-01", "x-api-key" = ${tomlQuote(apiKey)} }`,
        ]
      : [];
  return [
    ...
    `api_backend = ${tomlQuote(apiBackend)}`,
    'stream_tool_calls = false',
